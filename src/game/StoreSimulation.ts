...
export class StoreSimulation {
  ...
  closeDay(day: number): DaySnapshot {
    ...
    return {
      ...
      profit: this.metrics.profit - start.profit,
  ...
  createShoppingPlan(buildings: PlacedBuilding[]) {
    ...
    return shuffled.slice(0, shelfCount).map(shelf => ({
      shelf,
      requestedQuantity: PhaserMathBetween(1, 3),
    }))
  }
  ...
  restockAll() {
    ...
    for (const shelf of this.shelves.values()) {
      cost += (shelf.capacity - shelf.stock) * shelf.product.purchasePrice
    }
  ...
  finishCheckout(
    ...
    this.metrics.profit += basket.saleTotal - basket.purchaseTotal
