...
import { createEquipmentInventory, getProductCapacity, isProductCompatible, } from './equipment/EquipmentInventory';
export const INITIAL_BUDGET = 2_000;
export class StoreSimulation {
    inventories = new Map();
    shelfDefinitions = new Map();
    checkoutQueues = new Map();
    checkoutBusy = new Set();
    buildings = [];
    dayStart;
    currentDay = 1;
    nextAnonymousCustomer = 1;
    reserve = new ReserveManager(productCatalogReader);
    purchaseOrders = new PurchaseOrderManager(productCatalogReader, supplierCatalogReader);
    demand = new MarketDemandManager();
    customerAnalytics = new CustomerAnalyticsManager();
    metrics = {
    ...
    setCurrentDay(day) { this.currentDay = Math.max(1, Math.floor(day)); }
    syncBuildings(buildings) {
        ...
        for (const id of this.shelfDefinitions.keys())
            if (!shelfIds.has(id))
                this.shelfDefinitions.delete(id);
        for (const id of this.checkoutQueues.keys())
            if (!checkoutIds.has(id))
                this.checkoutQueues.delete(id);
        for (const shelf of shelves) {
            const definition = shelf.definition;
            this.shelfDefinitions.set(shelf.id, definition);
            if (!this.inventories.has(shelf.id))
                this.inventories.set(shelf.id, createEquipmentInventory(shelf.id, definition));
        }
        for (const checkout of checkouts)
            if (!this.checkoutQueues.has(checkout.id))
                this.checkoutQueues.set(checkout.id, []);
    }
    startDay() { this.dayStart = { ...this.metrics }; }
    closeDay(day, buildings = []) {
    ...
    canSpend(amount) { return this.metrics.cash >= amount; }
    spend(amount) {
    ...
    getCompatibleProducts(buildingId) {
    ...
    assignProductToCompartment(buildingId, compartmentId, productKey) {
        ...
        if (!product || !isProductCompatible(definition, product))
            return false;
    ...
    createPurchaseOrder(supplierKey, lines, day) {
    ...
    processDeliveries(day) { this.purchaseOrders.process(day, this.buildings, this.reserve); }
    ...
    getReserveQuantity(productKey) { return this.reserve.getQuantity(productKey); }
    getStorageCapacity(type) { return this.reserve.getCapacity(this.buildings, type); }
    getStorageUsed(type) { return this.reserve.getUsed(type); }
    getStorageFree(type) { return this.reserve.getFree(this.buildings, type); }
    hasStorage(type) { return this.getStorageCapacity(type) > 0; }
    getAvailableShelves(buildings) {
    ...
    takeItems(shelfId, compartmentId, requestedQuantity, context = {}) {
    ...
    choosePaymentMethod(accepted) {
        const available = accepted?.length ? accepted : ['contactless', 'card', 'cash'];
        const roll = Math.random(), preferred = roll < .5 ? 'contactless' : roll < .85 ? 'card' : 'cash';
        return available.includes(preferred) ? preferred : available[Math.floor(Math.random() * available.length)];
    }
    chooseCheckout(checkouts, basket, payment) {
        return checkouts.filter(checkout => {
            if (!isCheckoutDefinition(checkout.definition))
                return false;
            if (basket && checkout.definition.maxBasketSize !== undefined && basket.articleCount > checkout.definition.maxBasketSize)
                return false;
    ...
    getCheckoutTiming(checkout, articleCount, payment) {
        if (!isCheckoutDefinition(checkout.definition))
            return { durationMs: 0, incident: false };
        ...
        if (incident)
            this.metrics.checkoutIncidents += 1;
    ...
    getPickupTimeMs(shelf) { return isShelfDefinition(shelf.definition) ? shelf.definition.customerPickupTimeMs : 650; }
    enqueue(checkoutId, customerId) {
    ...
    isFirst(checkoutId, customerId) { return this.checkoutQueues.get(checkoutId)?.[0] === customerId && !this.checkoutBusy.has(checkoutId); }
    startCheckout(checkoutId) { this.checkoutBusy.add(checkoutId); }
    isCheckoutBusy(checkoutId) { return this.checkoutBusy.has(checkoutId); }
    finishCheckout(checkoutId, customerId, basket, payment, queueTimeMs, satisfaction) {
        const queue = this.checkoutQueues.get(checkoutId) ?? [];
        ...
        if (payment === 'contactless')
            this.metrics.contactlessPayments += 1;
        if (payment === 'card')
            this.metrics.cardPayments += 1;
        if (payment === 'cash')
            this.metrics.cashPayments += 1;
        for (const line of basket.lines)
            gameEvents.emit('product:sold', {
    ...
    abandon(checkoutId, customerId, satisfaction = 0) {
        if (checkoutId) {
            const queue = this.checkoutQueues.get(checkoutId) ?? [];
            const index = queue.indexOf(customerId);
            if (index >= 0)
    ...
    queueLength(checkoutId) { return this.checkoutQueues.get(checkoutId)?.length ?? 0; }
    queuePosition(checkoutId, customerId) { return this.checkoutQueues.get(checkoutId)?.indexOf(customerId) ?? -1; }
    getQueue(checkoutId) { return [...(this.checkoutQueues.get(checkoutId) ?? [])]; }
    getCompartment(buildingId, compartmentId) { return this.inventories.get(buildingId)?.compartments.find(item => item.id === compartmentId); }
    getEquipmentInventory(buildingId) { return this.inventories.get(buildingId); }
    ...
    applyDailyOperatingCosts(buildings) {
        const cost = buildings.reduce((total, building) => {
            if (isShelfDefinition(building.definition) || isStorageDefinition(building.definition))
                return total + (building.definition.electricityCostPerDay ?? 0);
            return total;
        }, 0);
        if (cost <= 0 || !this.canSpend(cost))
            return;
        this.metrics.cash -= cost;
        this.metrics.operatingExpenses += cost;
        this.recalculateProfit();
    }
    recalculateProfit() { this.metrics.profit = this.metrics.revenue - this.metrics.constructionExpenses - this.metrics.merchandiseExpenses - this.metrics.operatingExpenses; }
    emptySnapshotStart() {
