import type { PurchaseDecision, PurchaseDecisionResult } from '../commerce/MarketDemandManager'
...
export class CustomerAnalyticsManager {
  ...
  getRecent(limit = 30) {
    return this.observations.slice(-Math.max(0, limit)).reverse().map(item => ({ ...item }))
  }

  exportState(): CustomerAnalyticsState {
    return { observations: this.observations.map(item => ({ ...item })) }
  }

  importState(state?: CustomerAnalyticsState) {
    this.observations = (state?.observations ?? []).slice(-MAX_OBSERVATIONS).map(item => ({ ...item }))
  }

  clear() { this.observations = [] }

  private filter(day?: number) {
    return day === undefined ? this.observations : this.observations.filter(item => item.day === day)
  }
}
