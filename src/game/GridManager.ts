...
export class GridManager {
  ...
  constructor(public columns: number, public rows: number, public tileWidth: number, public tileHeight: number, public originX: number, public originY: number) {}
  get rotation() { return this.viewRotation }
  rotateView(step: -1 | 1) { this.viewRotation = ((this.viewRotation + step + 4) % 4) as Direction; return this.viewRotation }
  getViewDirection(direction: Direction): Direction { return ((direction + this.viewRotation) % 4) as Direction }
  ...
  getSize(definition: BuildingDefinition, direction: Direction) { return direction % 2 === 1 ? { width: definition.height, height: definition.width } : { width: definition.width, height: definition.height } }
  getFootprint(definition: BuildingDefinition, x: number, y: number, direction: Direction) {
    if (isEdgeDefinition(definition)) return []
    const size = this.getSize(definition, direction), cells: GridCell[] = []
    for (let dy = 0; dy < size.height; dy++) for (let dx = 0; dx < size.width; dx++) cells.push({ x: x + dx, y: y + dy })
    return cells
  }
  ...
  getBuildingAt(x: number, y: number) { const id = this.occupied.get(`${x}:${y}`); return id ? this.buildings.get(id) : undefined }
  ...
  place(definition: BuildingDefinition, x: number, y: number, direction: Direction) {
    ...
    if (isEdgeDefinition(definition)) {
      ...
      const edge: PlacedEdge = { id: crypto.randomUUID(), type: definition.category, definitionKey: definition.key, gridX: x, gridY: y, axis: this.getEdgeAxis(direction), direction }
  ...
  getBuildings(category?: Extract<BuildingCategory, 'shelf' | 'checkout' | 'storage'>) {
