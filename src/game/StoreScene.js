...
export class StoreScene extends Phaser.Scene {
    grid = new GridManager(16, 16, 64, 32, 700, 80);
    simulation = new StoreSimulation();
    customers = new Map();
    autoSpawn = false;
    day = 1;
    currentMinutes = OPENING_MINUTES;
    selectedBuildingId = null;
    navigation = new NavigationGrid(this.grid);
    selected = BUILDING_CATALOG.standardShelf;
    direction = 0;
    hovered = { x: -1, y: -1 };
    nextCustomer = 1;
    storeOpen = true;
    ...
    update(_time, delta) {
        ...
        if (this.cursors?.up.isDown || this.cameraKeys?.up.isDown)
            camera.scrollY -= amount;
        if (this.cursors?.down.isDown || this.cameraKeys?.down.isDown)
            camera.scrollY += amount;
        if (this.cursors?.left.isDown || this.cameraKeys?.left.isDown)
            camera.scrollX -= amount;
        if (this.cursors?.right.isDown || this.cameraKeys?.right.isDown)
            camera.scrollX += amount;
    }
    select(key) {
    ...
    configureCompartment(buildingId, compartmentId, productKey) {
    ...
    rotateScene(step) {
        if (this.customers.size > 0)
            return this.setStatus('Attendez le départ des clients avant de tourner la scène.', '#fbbf24');
    ...
    toggleAutoSpawn() {
        if (!this.storeOpen)
            return this.setStatus('Le magasin est fermé.', '#f87171');
    ...
    bindPointerControls() {
        this.input.on('pointermove', (pointer) => {
            ...
            if (this.isDragging && pointer.leftButtonDown() && this.selected.category === 'wall')
                this.placeDraggedWall();
            this.drawPreview();
        });
        this.input.on('pointerdown', (pointer) => {
            ...
            if (!this.grid.isInside(this.hovered.x, this.hovered.y))
                return;
            const existing = this.grid.getBuildingAt(this.hovered.x, this.hovered.y);
            if (pointer.rightButtonDown()) {
                if (existing?.id === this.selectedBuildingId)
    ...
    bindKeyboardControls() {
        this.input.keyboard?.on('keydown-R', () => { this.direction = ((this.direction + 1) % 4); this.drawPreview(); });
    ...
    startClock() {
        ...
        this.clockTimer = this.time.addEvent({ delay: CLOCK_STEP_MS, loop: true, callback: () => {
                ...
                if (this.currentMinutes >= CLOSING_MINUTES)
                    this.closeStore();
            } });
    }
    closeStore() {
    ...
    placeSelected() {
        ...
        if (placed) {
            ...
            this.syncSimulation();
            if ('definition' in placed)
                this.selectedBuildingId = placed.id;
        }
    }
    placeDraggedWall() {
        const key = `${this.hovered.x}:${this.hovered.y}:${this.direction % 2}`;
        if (key === this.lastDragKey || !this.grid.isInside(this.hovered.x, this.hovered.y))
            return;
    ...
    syncSimulation() { this.simulation.syncBuildings(this.grid.getBuildings()); }
    findCustomerEntry(firstShelf) {
    ...
    async runCustomerCycle(customer, shoppingPlan, route) {
        let queuedCheckout;
        ...
        try {
            ...
            if (!checkout) {
                ...
                if (!checkout || !isCheckoutDefinition(checkout.definition))
                    throw new Error('no-compatible-checkout');
            ...
            while (!this.simulation.isFirst(checkout.id, customer.id)) {
                ...
                if (queueTime >= MAX_QUEUE_WAIT_MS)
                    throw new Error('impatient');
                await this.wait(350);
            }
            ...
            if (queuedCheckout)
                await this.repositionQueue(queuedCheckout);
        }
        finally {
            customer.destroy();
    ...
    paymentLabel(payment) { return payment === 'contactless' ? 'sans contact' : payment === 'card' ? 'carte' : 'espèces'; }
    async repositionQueue(checkout) {
    ...
    async moveCustomerToCheckout(customer, checkout) {
        const adjacent = this.grid.getAdjacentWalkableCells(checkout)[0];
        if (!adjacent)
            return;
    ...
    drawPath(path) {
        ...
        path.forEach((cell, index) => {
            ...
            if (index === 0)
                this.pathLayer.moveTo(p.x, p.y + 16);
            else
                this.pathLayer.lineTo(p.x, p.y + 16);
        });
    ...
    drawPreview() {
        ...
        if (!isEdgeTool && this.grid.getBuildingAt(this.hovered.x, this.hovered.y))
            return;
    ...
    drawBuilding(building) {
        ...
        if (renderer === 'fruit-shelf')
            return this.drawShelfBoxes(building, 26, 0x84cc16, true);
        if (renderer === 'refrigerated-shelf')
            return this.drawColdShelf(building, false);
        if (renderer === 'freezer')
            return this.drawColdShelf(building, true);
        if (renderer === 'bakery-shelf')
            return this.drawShelfBoxes(building, 32, 0xc08457, true);
        if (renderer === 'self-checkout')
            return this.drawCheckout(building, 20, 0x06b6d4);
        if (renderer === 'express-checkout')
            return this.drawCheckout(building, 22, 0xf59e0b);
        if (renderer === 'standard-checkout')
            return this.drawCheckout(building, 28, 0x2563eb);
        this.drawShelfBoxes(building, 44, building.definition.color);
    }
    drawColdShelf(building, frozen) {
    ...
    drawShelfBoxes(building, height, color, open = false) {
        ...
        for (const cell of this.grid.getFootprint(building.definition, building.gridX, building.gridY, building.direction)) {
            ...
            if (open)
                this.buildingsLayer.fillStyle(0xfef3c7, .65).fillEllipse(p.x, p.y - height + 8, 28, 12);
        }
        if (inventory) {
    ...
    drawCheckout(building, height, color) {
        for (const cell of this.grid.getFootprint(building.definition, building.gridX, building.gridY, building.direction)) {
            const p = this.grid.gridToScreen(cell.x, cell.y);
    ...
    drawEdge(edge) {
        ...
        if (edge.type === 'wall')
            this.buildingsLayer.fillStyle(0xcbd5e1).beginPath().moveTo(start.x, start.y).lineTo(end.x, end.y).lineTo(end.x, end.y - 46).lineTo(start.x, start.y - 46).closePath().fillPath();
        else
            this.buildingsLayer.lineStyle(5, 0x7c3aed).beginPath().moveTo(start.x, start.y).lineTo(start.x, start.y - 42).moveTo(end.x, end.y).lineTo(end.x, end.y - 42).strokePath();
    }
    diamond(g, x, y, color, alpha, line) {
    ...
    wait(duration) { return new Promise(resolve => this.time.delayedCall(duration, resolve)); }
    setStatus(message, color = '#cbd5e1') { this.statusLabel.setText(message).setColor(color); }
    textStyle(fontSize, color = '#ffffff') {
