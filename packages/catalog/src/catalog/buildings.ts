...
function validateBuilding(definition: BuildingDefinition, filename: string) {
  ...
  if (definition.category === 'storage') {
    if (definition.capacity <= 0) throw new Error(`${definition.key} : capacité de réserve invalide.`)
  }
...
export const STORAGE_BUILDINGS = BUILDINGS.filter(item => item.category === 'storage')
