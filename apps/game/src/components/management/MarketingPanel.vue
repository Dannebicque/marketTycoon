  ...
  <div class="management-content marketing-layout">
    <section class="promotion-form">
      ...
      <div v-if="preview" class="promotion-preview">
        ...
        <div><span>Coût de campagne</span><strong>{{ money(preview.campaignCost) }}</strong></div>
      </div>
    ...
    <section>
      ...
      <article v-for="promotion in promotions" :key="promotion.id" class="promotion-card" :class="status(promotion)">
        ...
        <div class="campaign-channel"><strong>{{ promotionManager.getChannelLabel(promotion.channel) }}</strong><small>Coût {{ money(promotion.campaignCost) }}</small></div>
        ...
        <details class="campaign-report"><summary>Analyse avant / pendant / après</summary>
          ...
          <dl class="impact-list"><div><dt>Volume supplémentaire estimé</dt><dd>{{ signed(report(promotion).incrementalQuantity) }}</dd></div><div><dt>CA supplémentaire estimé</dt><dd>{{ signedMoney(report(promotion).incrementalRevenue) }}</dd></div><div><dt>Marge sacrifiée</dt><dd>{{ money(report(promotion).sacrificedMargin) }}</dd></div><div><dt>Coût marketing</dt><dd>{{ money(report(promotion).campaignCost) }}</dd></div><div><dt>Impact net estimé</dt><dd :class="report(promotion).estimatedNetImpact >= 0 ? 'positive-text' : 'negative-text'">{{ signedMoney(report(promotion).estimatedNetImpact) }}</dd></div><div><dt>ROI</dt><dd>{{ percent(report(promotion).roi) }}</dd></div></dl>
        </details>
...
const preview = computed(() => {
  ...
  const synthetic = promotionManager.schedule
  void synthetic
...
function schedule() { if (!canSubmit.value) return; const promotion = promotionManager.schedule({ ...form }); if (!promotion) return; persistPromotions(); version.value += 1 }
