  ...
  <section class="commercial-zone-widget" :class="{ open }">
    ...
    <div v-if="open" class="commercial-zone-panel">
      ...
      <div class="sector-create">
        <select v-model="sectorKey"><option v-for="sector in definitions" :key="sector.key" :value="sector.key">{{ sector.icon }} {{ sector.name }}</option></select>
        <input v-model="zoneName" placeholder="Nom de la zone" @keyup.enter="createZone" />
        <button @click="createZone">Créer</button>
      </div>
      <div class="custom-sector">
        <input v-model="customName" placeholder="Nouveau secteur personnalisé" @keyup.enter="createSector" />
...
function createZone() { if (!sectorKey.value) return; const zone = commercialZoneRuntime.createZone(sectorKey.value, zoneName.value); if (zone) zoneName.value = '' }
...
function select(id: string) { commercialZoneRuntime.select(id) }
function selectEraser() { commercialZoneRuntime.selectEraser() }
