<template>
  <aside v-if="parcel" class="parcel-widget" aria-live="polite">
    ...
    <div v-if="parcel.price" class="parcel-price">{{ money(parcel.price) }}</div>
...
const stateMessage = computed(() => {
  ...
  if (parcel.value.access === 'owned') return 'Terrain possédé'
...
function handleSelected(event: Event) {
  const detail = (event as CustomEvent<{ parcel: SelectedParcel | null; canPurchase: boolean }>).detail
  parcel.value = detail?.parcel ?? null
  canPurchase.value = detail?.canPurchase ?? false
  message.value = ''
}

function handlePurchaseResult(event: Event) {
...
onMounted(() => {
  ...
  window.addEventListener('market-tycoon:parcel-purchase-result', handlePurchaseResult)
})
onBeforeUnmount(() => {
  window.removeEventListener('market-tycoon:parcel-selected', handleSelected)
  window.removeEventListener('market-tycoon:parcel-purchase-result', handlePurchaseResult)
})
</script>
